	...
	def __init__(self, graph, colors={1:'eax',2:'ebx',3:'ecx',4:'edx',5: 'edi', 6:'esi'}):
		...
		for node in self.theGraph.keys():
			if (not isinstance(node, VarNode)) or node.color != -1:
				continue
			self.Queue.add_task(len(self.theGraph[node]),node)
	def doColor(self):
                print "chaitin docolor"
		curNode = None
		myColorStack = []
		mySpillStack = []
			
		while not self.Queue.empty():
			curNode = self.Queue.pop_task() 
			myLen = 0
			try:
